...
#    define EE_EXTENDED_PORT_0A 27
#    define EE_EXTENDED_PORT_0B 28
#    define EE_EXTENDED_PORT_1A 29
#    define EE_EXTENDED_PORT_1B 30
#    define EE_EXTENDED_PORT_2A 31
#    define EE_EXTENDED_PORT_2B 32
#    define EE_EXTENDED_PORT_3A 33
#    define EE_EXTENDED_PORT_3B 34
#    define EE_EXTENDED_PORT_4A 35
#    define EE_EXTENDED_PORT_4B 36
#    define EE_EXTENDED_PORT_5A 37
#    define EE_EXTENDED_PORT_5B 38
#    define EE_EXTENDED_PORT_6A 39
#    define EE_EXTENDED_PORT_6B 40
#    define EE_EXTENDED_PORT_7A 41
#    define EE_EXTENDED_PORT_7B 42
    /* de all_of functie
     ...
     *
     */
#    define EE_all_of_E_MICROCONTROLLER_PORTA 43
#    define EE_all_of_E_MICROCONTROLLER_PORTB 44
#    define EE_all_of_E_MICROCONTROLLER_PORTC 45
#    define EE_all_of_E_MICROCONTROLLER_PORTD 46
#    define EE_all_of_E_EXTENDED_PORT_0A 47
#    define EE_all_of_E_EXTENDED_PORT_0B 48
#    define EE_all_of_E_EXTENDED_PORT_1A 49
#    define EE_all_of_E_EXTENDED_PORT_1B 50
#    define EE_all_of_E_EXTENDED_PORT_2A 51
#    define EE_all_of_E_EXTENDED_PORT_2B 52
#    define EE_all_of_E_EXTENDED_PORT_3A 53
#    define EE_all_of_E_EXTENDED_PORT_3B 54
#    define EE_all_of_E_EXTENDED_PORT_4A 55
#    define EE_all_of_E_EXTENDED_PORT_4B 56
#    define EE_all_of_E_EXTENDED_PORT_5A 57
#    define EE_all_of_E_EXTENDED_PORT_5B 58
#    define EE_all_of_E_EXTENDED_PORT_6A 59
#    define EE_all_of_E_EXTENDED_PORT_6B 60
#    define EE_all_of_E_EXTENDED_PORT_7A 61
#    define EE_all_of_E_EXTENDED_PORT_7B 62

#    define EE_all_of_MICROCONTROLLER_PORTA 63
#    define EE_all_of_MICROCONTROLLER_PORTB 64
#    define EE_all_of_MICROCONTROLLER_PORTC 65
#    define EE_all_of_MICROCONTROLLER_PORTD 66
#    define EE_all_of_EXTENDED_PORT_0A 67
#    define EE_all_of_EXTENDED_PORT_0B 68
#    define EE_all_of_EXTENDED_PORT_1A 69
#    define EE_all_of_EXTENDED_PORT_1B 70
#    define EE_all_of_EXTENDED_PORT_2A 71
#    define EE_all_of_EXTENDED_PORT_2B 72
#    define EE_all_of_EXTENDED_PORT_3A 73
#    define EE_all_of_EXTENDED_PORT_3B 74
#    define EE_all_of_EXTENDED_PORT_4A 75
#    define EE_all_of_EXTENDED_PORT_4B 76
#    define EE_all_of_EXTENDED_PORT_5A 77
#    define EE_all_of_EXTENDED_PORT_5B 78
#    define EE_all_of_EXTENDED_PORT_6A 79
#    define EE_all_of_EXTENDED_PORT_6B 80
...
#    warning "PROTOCOL_VERSIE not defined"
    ...
    static void set_port(uint8_t uitgang, uint8_t state, uint8_t duur)
    {
        ...
        else if (uitgang < 0xA0)
        {
            /* EX port */
            uint8_t port_nr= uitgang-0x20;
            uint8_t slave=(port_nr >> 4) & 0x07;/* 0-7 */
            port_nr &= 0x0f;
            if(port_nr<0x08)
            {
                uint8_t port =MCP23S17_read_register(slave,GPIOA);

                if (state == 0x01) { port |= (1 << port_nr); }
                else if (state == 0x02)
                {
                    port ^= (1 << port_nr);
                    CAN_TX_msg.data_byte[2] = (port & (1 << port_nr));
                }
                else
                {
                    port &= ~(1 << port_nr);
                }
                MCP23S17_write_register(slave,GPIOA, port);
                if(MCP23S17_read_register(slave,GPIOA)!=port){
                    /* error */
                CAN_TX_msg.length       = 5;
                ...
                port_nr -= 0x08;
                uint8_t port =MCP23S17_read_register(slave,GPIOB);
                ...
                MCP23S17_write_register(slave,GPIOB, port);
                if(MCP23S17_read_register(slave,GPIOB)!=port){
                    /* error */
                CAN_TX_msg.length       = 5;
                CAN_TX_msg.data_byte[0] = 0x00; /* error in code µc */
                CAN_TX_msg.data_byte[1] = 0x03; /* 1 uitgang */
    ...
    {
        if(RingBuffer_IsEmpty(&RX_Buffer_1)) return;
    ...
    static void new_build_USART_data_block()
    {
        ...
        if(status==0x00 && RingBuffer_Peek(&RX_Buffer_1) == EBUSD_SYN)
        {
            ...
            crc=ebusd_crc(crc,EBUSD_telegram_master.QQ);
            status=0x01;// QQ ok
        }
        ...
        if(status==0x02)
        {
            ...
            crc=ebusd_crc(crc,EBUSD_telegram_master.PB);
            status=0x03;// QQ ZZ PB ok
        }
        ...
        if(status==0x03)
        {
            ...
            crc=ebusd_crc(crc,EBUSD_telegram_master.SB);
            status=0x04;// QQ ZZ PB SB ok
        }
        ...
        if(status==0x04)
        {
            ...
            crc=ebusd_crc(crc,EBUSD_telegram_master.NN);
            status=0x05;// QQ ZZ PB SB NN ok
        }

        if(RingBuffer_IsEmpty(&RX_Buffer_1)) return;

        if(status>0x04 && status<(EBUSD_telegram_master.NN+0x05))
        {
            if(RingBuffer_Peek(&RX_Buffer_1) == EBUSD_SYN){
                status=0x00;
                return;
            ...
            // A9 = A9 00
            // AA = A9 01
            uint8_t temp_var=RingBuffer_Peek(&RX_Buffer_1);
            if(temp_var==0xa9){
                if(RingBuffer_GetCount(&RX_Buffer_1)<2){
                    return;// er moeten 2 byts zijn
                }
                RingBuffer_Remove(&RX_Buffer_1);
                if(RingBuffer_Remove(&RX_Buffer_1)==0x01){
                    temp_var=0xaa;
                    crc=ebusd_crc(crc,0xa9);
                    crc=ebusd_crc(crc,0x01);
                } else {
                    temp_var=0xa9;
                    crc=ebusd_crc(crc,0xa9);
                    crc=ebusd_crc(crc,0x00);
                }
            } else {
                RingBuffer_Remove(&RX_Buffer_1);
                crc=ebusd_crc(crc,temp_var);
            }
            EBUSD_telegram_master.data_byte[(status-0x05)]=temp_var;
            ++status;// QQ ZZ PB SB NN ok get data
        }

        if(RingBuffer_IsEmpty(&RX_Buffer_1)) return;

        if(status==(EBUSD_telegram_master.NN+0x05))
        {
            EBUSD_telegram_master.CRC=RingBuffer_Remove(&RX_Buffer_1);
            crc=ebusd_crc(crc,EBUSD_telegram_master.CRC);
            if(crc!=0x00){
                status=0x00;
                return;
            }
            status=0x80;// QQ ZZ PB SB NN data master ok
        }
        ...
        if(status==0x80)
        {
            ...
            {
                //is een brotcast
                uint8_t lenkte=EBUSD_telegram_master.NN;
                if(lenkte>7){
                    lenkte=7;
                }
                ...
                CAN_TX_msg.ext_id       = CAN_EXTENDED_FRAME;
                CAN_TX_msg.rtr          = 0;
                CAN_TX_msg.length       = lenkte+1;
                CAN_TX_msg.data_byte[0] = 0xA0;
                uint8_t val=0;
                for (;val<lenkte;++val) {
                ...
                if(EBUSD_telegram_master.NN>7){
                    lenkte=EBUSD_telegram_master.NN-7;
                    if(lenkte>7){
                        lenkte=7;
                    }
                    CAN_TX_msg.id           = ebusd_To_CAN_id();
                    ...
                    CAN_TX_msg.rtr          = 0;
                    CAN_TX_msg.length       = lenkte+1;
                    CAN_TX_msg.data_byte[0] = 0xA1;
                    uint8_t val=0;
                    for (;val<lenkte;++val) {
                        CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.data_byte[val+7];
                    }
                    while (MCP2515_message_TX()==0) {
                        //0==Error no Transmit buffer empty
                    }
                ...
                if(EBUSD_telegram_master.NN>7){
                    ...
                    CAN_TX_msg.rtr          = 0;
                    CAN_TX_msg.length       = lenkte+1;
                    CAN_TX_msg.data_byte[0] = 0xA2;
                    uint8_t val=0;
                    for (;val<lenkte;++val) {
                        CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.data_byte[val+14];
                    }
                    while (MCP2515_message_TX()==0) {
                        //0==Error no Transmit buffer empty
                    }
            ...
            while (length>0) {
                ...
                CAN_TX_msg.ext_id       = CAN_EXTENDED_FRAME;
                CAN_TX_msg.rtr          = 0;
                CAN_TX_msg.length       = te_versturen+1;
                CAN_TX_msg.data_byte[0] = 0xF0+can_Number;
                uint8_t val=0;
                for (;val<te_versturen;++val) {
                    CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.data_byte[val+(can_Number*7)];
                }
                while (MCP2515_message_TX()==0) {
                    //0==Error no Transmit buffer empty
                }
            ...
            status=0x81;// QQ ZZ PB SB NN data master CRC ACK ok
        }
        ...
        if(status==0x81)
        {
            ...
            {
                RingBuffer_Remove(&RX_Buffer_1);
                //is een master-master
                status=0x00;
                return;
            }
            EBUSD_telegram_master.Slave_NN=RingBuffer_Remove(&RX_Buffer_1);
            crc=0x00;//reset crc
            crc=ebusd_crc(crc,EBUSD_telegram_master.Slave_NN);
            status=0x82;// QQ ZZ PB SB NN ok
        }

        if(RingBuffer_IsEmpty(&RX_Buffer_1)) return;

        if(status>0x81 && status<(EBUSD_telegram_master.Slave_NN+0x82))
        {
            if(RingBuffer_Peek(&RX_Buffer_1) == EBUSD_SYN){
                status=0x00;
                return;
            ...
            EBUSD_telegram_master.Slave_data_byte[(status-0x82)]=temp_var;
            ++status;// QQ ZZ PB SB NN ok get data
        }

        if(RingBuffer_IsEmpty(&RX_Buffer_1)) return;

        if(status==(EBUSD_telegram_master.Slave_NN+0x82))
        {
            EBUSD_telegram_master.Slave_CRC=RingBuffer_Remove(&RX_Buffer_1);
            crc=ebusd_crc(crc,EBUSD_telegram_master.Slave_CRC);
            if(crc!=0x00){
                status=0x00;
                return;
            }
            status=0xF0;// QQ ZZ PB SB NN data master ok
        }
        ...
        if(status==0xF0)
        {
            ...
            if(EBUSD_telegram_master.ACK==EBUSD_ACK_ok){
                uint8_t length=EBUSD_telegram_master.Slave_NN;
                uint8_t can_Number=0;
                while (length>0) {
                    uint8_t te_versturen=length;
                    if(te_versturen>7){
                    ...
                    CAN_TX_msg.rtr          = 0;
                    CAN_TX_msg.length       = te_versturen+1;
                    CAN_TX_msg.data_byte[0] = 0xf3+can_Number;
                    uint8_t val=0;
                    for (;val<te_versturen;++val) {
                        CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.Slave_data_byte[val+(can_Number*7)];
                    }
                    while (MCP2515_message_TX()==0) {
                        //0==Error no Transmit buffer empty
                    }
...
//            if(EBUSD_telegram_master.NN<17)
//            {
//                status=0x01;
//            }
//            else
...
//                    CAN_TX_msg.rtr          = 0;
//                    val=0;
//                    length=EBUSD_telegram_master.NN-7;
//                    if(length>7){
//                        length=7;
//                    }
//                    CAN_TX_msg.length       = length+1;
//                    CAN_TX_msg.data_byte[0] = 0xf1;
//                    for (;val<length;++val) {
//                        CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.data_byte[val+7];
//                    }
//                    while (MCP2515_message_TX()==0) {
...
//            }
//            EBUSD_telegram_master.Slave_NN=RingBuffer_Remove(&RX_Buffer_1);
//            if(EBUSD_telegram_master.Slave_NN>17){
//                EBUSD_telegram_master.Slave_NN=0x00;
//                status=0x0f;
//                return;
//            }

//            uint8_t val=0;
//            for (;val<EBUSD_telegram_master.Slave_NN;++val) {
//                EBUSD_telegram_master.Slave_data_byte[val]=RingBuffer_Remove(&RX_Buffer_1);
//            }
//            EBUSD_telegram_master.Slave_CRC=RingBuffer_Remove(&RX_Buffer_1);
//            EBUSD_telegram_master.ACK=RingBuffer_Remove(&RX_Buffer_1);
//            if(EBUSD_telegram_master.ACK==EBUSD_ACK_ok){
...
//                for (;val<length;++val) {
//                    CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.Slave_data_byte[val];
//                }
//                while (MCP2515_message_TX()==0) {
//                    //0==Error no Transmit buffer empty
//                }
//                if(EBUSD_telegram_master.Slave_NN>7){
//                    CAN_TX_msg.id           = ebusd_To_CAN_id();
//                    CAN_TX_msg.ext_id       = CAN_EXTENDED_FRAME;
//                    CAN_TX_msg.rtr          = 0;
//                    val=0;
//                    length=EBUSD_telegram_master.Slave_NN-7;
//                    if(length>7){
//                        length=7;
//                    }
//                    CAN_TX_msg.length       = length+1;
//                    CAN_TX_msg.data_byte[0] = 0xf4;
//                    for (;val<length;++val) {
//                        CAN_TX_msg.data_byte[val+1] = EBUSD_telegram_master.Slave_data_byte[val+7];
//                    }
//                    while (MCP2515_message_TX()==0) {
//                        //0==Error no Transmit buffer empty
//                    }
...
//    }
    ...
    int main(void)
    {
        ...
        uint8_t v_pin_EX_A[8];
        uint8_t v_pin_EX_B[8];
        for (uint8_t var=0; var<8; ++var) {
            #ifdef PCB_dev
                uint8_t slave_address[8]={0x00,0x04,0x02,0x06,0x01,0x05,0x03,0x07};/* Fix PCB fout */
                uint8_t OpCode = (uint8_t)(slave_address[var] << 0x01);
            #else
                uint8_t slave_address=var;
                slave_address &= 0x07;
                uint8_t OpCode = (uint8_t)(slave_address << 0x01);
            #endif
            OpCode |= 0x41; /* read */
            ...
            spi_write(OpCode);
            spi_write(GPIOA);
            uint8_t pin_EX_A= spi_read();//GPIOA
            uint8_t pin_EX_B= spi_read();//GPIOB
            MCP23S17_UNSELECT();

            MCP23S17_SELECT();
            spi_write(OpCode);
            spi_write(IODIRA);
            uint8_t DDR_EX_A= spi_read();//IODIRA
            uint8_t DDR_EX_B= spi_read();//IODIRB
            MCP23S17_UNSELECT();

            v_pin_EX_A[var] = (pin_EX_A & DDR_EX_A);
            v_pin_EX_B[var] = (pin_EX_B & DDR_EX_B);
        }
        ...
        for (uint8_t var=0; var<8; ++var) {
            uint8_t offset =(uint8_t)(var<<4);
            offset +=0x20;
            v_pin_EX_A[var] = input_pol(v_pin_EX_A[var],~v_pin_EX_A[var],offset);
            offset +=0x08;
            v_pin_EX_B[var] = input_pol(v_pin_EX_B[var],~v_pin_EX_B[var],offset);
        }
        ...
        for (;;)
        {
            ...
            if ((TCNT3 & 0x1000) && poling==0){
                ...
                for (uint8_t var=0; var<8; ++var) {
                    ...
                    v_pin_EX_A[var] = input_pol((pin_EX_A & DDR_EX_A),v_pin_EX_A[var],offset);
                    offset +=0x08;
                    v_pin_EX_B[var] = input_pol((pin_EX_B & DDR_EX_B),v_pin_EX_B[var],offset);
                }
